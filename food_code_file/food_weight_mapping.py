...
if __name__ == '__main__':
    ...
    raw_food_code_weight = pd.read_csv('raw_food_code_weight.csv')
    ...
    single_food_name_weight = pd.read_csv('single_food_name_weight.csv')
    single_food_code_weight = pd.read_csv('single_food_code_weight.csv')
    ...
    for index,row in tqdm(raw_food_code_weight.iterrows()):
        ...
        weight = single_food_name_weight[single_food_name_weight["ingredient"] == row["ingredient"]]["weight"]
        ...
        if np.isnan(weight.values[0]):
            ...
            if str(row["food_code_1"]) == "nan":
                print("need to change manually: ", row.tolist())
                list_to_csv(manual_file_name, [row.tolist()])
                continue
            else:
                weight = single_food_code_weight[single_food_code_weight["food_code_1"] == row["food_code_1"]]["weight"]
